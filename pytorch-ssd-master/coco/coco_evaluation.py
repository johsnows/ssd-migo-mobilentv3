...
def coco_evaluation(dataset, predictions, output_dir, iteration=None):
    coco_results = []
    now_predictions=predictions
    # for _, p in enumerate(predictions):
    #     if _ <10: print(p)
    #     now_predictions.update(p)
    # print(now_predictions.keys())
    image_ids=list(sorted(now_predictions.keys()))
    predictions=[now_predictions[i] for i in image_ids]
